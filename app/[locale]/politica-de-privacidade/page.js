// app/[locale]/politica-de-privacidade/page.js
...
import PoliticaPrivacidadeClient from "@/app/politica-de-privacidade/PoliticaPrivacidadeClient";

export async function generateMetadata({ params }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "privacidade.meta" });
  const ogLocale = locale === "en" ? "en_GB" : "pt_PT";
  const canonicalPath = locale === "en" ? `${SITE_URL}/en/privacy-policy` : `${SITE_URL}/politica-de-privacidade`;
