...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section style={{ height: '100vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ zIndex: 2, textAlign: 'center', color: '#fff', padding: '0 20px' }}>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            style={{ fontSize: 'clamp(3.5rem, 12vw, 6.5rem)', marginBottom: '0' }}>
            Flores à Beira-Rio
          </motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            style={{ fontSize: '1.1rem', letterSpacing: '4px', textTransform: 'uppercase', opacity: 0.9, marginTop: '20px', fontWeight: '400' }}>
            Especialistas em preservação de flores
          </motion.p>
        </div>
      </section>

      {/* 2. OS 3 PASSOS (Agora em Segundo Lugar) */}
      ...
      {/* 3. GOOGLE REVIEWS (Agora em Terceiro Lugar) */}
      ...
      {/* 4. SUSTENTABILIDADE & DIFERENCIAL */}
      ...
      <section style={{ padding: '120px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '100px' }}>
          <div>
            ...
            <button style={{ backgroundColor: '#1a1a1a', color: '#fff', padding: '20px 40px', border: 'none', cursor: 'pointer', width: '100%', letterSpacing: '2px', textTransform: 'uppercase', fontSize: '0.75rem', fontWeight: '600' }}>Oferecer Vale-Presente</button>
          </div>
          <div>
            ...
            <button style={{ backgroundColor: 'transparent', color: '#1a1a1a', padding: '20px 40px', border: '1px solid #1a1a1a', cursor: 'pointer', width: '100%', letterSpacing: '2px', textTransform: 'uppercase', fontSize: '0.75rem', fontWeight: '600' }}>Agendar Agora</button>
          </div>
        </div>
      </section>
