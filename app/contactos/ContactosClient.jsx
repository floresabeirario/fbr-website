...
function Reveal({ children, delay = 0, style, className }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 36 }} animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 36 }} transition={{ duration: 0.75, delay, ease: [0.25, 0.1, 0.25, 1] }} style={style} className={className}>
      {children}
    </motion.div>
  );
}
      ...
      <style dangerouslySetInnerHTML={{
        ...
        * { box-sizing: border-box; }
        ...
        @media (min-width: 640px) {
          .team-bio-col { display: none; }
          .team-card { flex-direction: column; }
          .team-photo-wrap { width: 100%; aspect-ratio: 3/4; }
          .team-bio-below { display: block; }
        }

        .team-bio-below { display: none; padding: 20px 20px 24px; }
