...
export default function RootLayout({ children }) {
  ...
  const logoSize = useTransform(scrollY, [0, 400], ["clamp(4rem, 15vw, 8rem)", "1.6rem"]);
    ...
    <html lang="pt">
      ...
      <body style={{ margin: 0, backgroundColor: '#FCFBF9', color: '#1a1a1a', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>
        ...
        <motion.nav style={{ 
          ...
          height: '80px', backgroundColor: navBg, display: 'flex', alignItems: 'center'
        }}>
          <div style={{ maxWidth: '1450px', margin: '0 auto', width: '100%', padding: '0 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            
            <div className="desktop-only" style={{ display: 'flex', gap: '30px', flex: 1 }}>
              {menuLeft.map((item) => (
                <motion.a key={item.name} href={item.href} className="nav-link" style={{ textDecoration: 'none', fontSize: '0.7rem', fontWeight: '500', textTransform: 'uppercase', letterSpacing: '2px', color: linkColor }}>
                  {item.name}
                </motion.a>
              ))}
            </div>
            ...
            <div className="desktop-only" style={{ display: 'flex', gap: '30px', flex: 1, justifyContent: 'flex-end' }}>
              {menuRight.map((item) => (
                <motion.a key={item.name} href={item.href} className="nav-link" style={{ textDecoration: 'none', fontSize: '0.7rem', fontWeight: '500', textTransform: 'uppercase', letterSpacing: '2px', color: linkColor, display: 'flex', alignItems: 'center' }}>
                  {item.name} {item.hasFlag && <FlagEN />}
                </motion.a>
              ))}
            </div>

            <button className="mobile-only nav-link" onClick={() => setIsOpen(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: linkColor, fontSize: '0.8rem', fontWeight: '500', letterSpacing: '2px' }}>
              MENU
            </button>
          </div>
        </motion.nav>
        ...
        <motion.div style={{ 
          ...
          top: logoTop, y: "-50%", textAlign: 'center', pointerEvents: 'none', 
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <button onClick={() => setIsOpen(false)} className="nav-link" style={{ position: 'absolute', top: '30px', right: '30px', background: 'none', border: 'none', fontSize: '0.8rem', letterSpacing: '2px', cursor: 'pointer' }}>FECHAR</button>
              {[...menuLeft, ...menuRight].map((item) => (
                <a key={item.name} href={item.href} onClick={() => setIsOpen(false)} className="nav-link" style={{ textDecoration: 'none', color: '#1a1a1a', fontSize: '2rem', margin: '15px 0', fontFamily: "'TAN-MEMORIES', serif", display: 'flex', alignItems: 'center' }}>
                  {item.name} {item.hasFlag && <FlagEN />}
                </a>
              ))}
            </motion.div>
        ...
        <style jsx global>{`
          ...
          /* Efeito de Hover Premium para os links do menu */
          .nav-link {
