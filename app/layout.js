...
// Ícone da Bandeira em SVG (para evitar o erro "GB")
const FlagEN = () => (
...
export default function RootLayout({ children }) {
  ...
  // --- ANIMAÇÃO GOSHA (O Título que viaja) ---
      ...
      <body style={{ margin: 0, backgroundColor: '#FCFBF9', color: '#1a1a1a', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>
        ...
        <motion.nav style={{ 
          ...
          <div style={{ maxWidth: '1450px', margin: '0 auto', width: '100%', padding: '0 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div className="desktop-only" style={{ display: 'flex', gap: '30px', flex: 1, justifyContent: 'flex-end' }}>
              {menuRight.map((item) => (
                <motion.a key={item.name} href={item.href} style={{ textDecoration: 'none', fontSize: '0.75rem', fontWeight: '500', textTransform: 'uppercase', letterSpacing: '2px', color: linkColor, display: 'flex', alignItems: 'center' }}>
        ...
        <motion.div style={{ 
          ...
          <motion.p style={{ 
            ...
            letterSpacing: '10px', fontSize: '1.1rem', marginTop: '20px', fontWeight: '300',
