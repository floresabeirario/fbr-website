...
const STEPS = [
  {
    ...
    noteJsx: (
      <>
        ...
        <a href={FORM_URL} style={{ color: "var(--rust)", fontWeight: 600, textDecoration: "none", borderBottom: "1px solid rgba(200,82,42,0.3)", paddingBottom: "1px" }}>
  ...
  },
  {
    ...
    noteJsx: (
      <>
        <strong style={{ display: "block", marginBottom: "10px", color: "var(--rust)", fontSize: "0.88rem" }}>Três opções de entrega:</strong>
  ...
  },
  {
    ...
    noteJsx: (
      <>
        <strong style={{ display: "block", marginBottom: "10px", color: "var(--rust)", fontSize: "0.88rem" }}>Com o quadro pronto, tem duas opções:</strong>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {[
    ...
    <article id={step.id} aria-labelledby={`${step.id}-title`} style={{ marginBottom: "clamp(64px,12vw,110px)" }}>
      ...
      >
        ...
        <div className="step-text">
          <h2 id={`${step.id}-title`} className="step-title-desktop" style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "clamp(1.6rem,4vw,2.5rem)", color: "var(--green-d)", margin: "0 0 clamp(12px,2vw,18px)", lineHeight: 1.1 }}>
            {step.title}
          </h2>
          ...
          {(step.note || step.noteJsx) && (
            <div style={{ padding: "clamp(14px,2vw,18px) clamp(16px,2.5vw,22px)", borderRadius: "12px", backgroundColor: "rgba(200,82,42,0.05)", borderLeft: "3px solid rgba(200,82,42,0.25)" }}>
              {step.noteJsx ? (
                <div style={{ color: "var(--rust)", lineHeight: 1.78, fontSize: "clamp(0.84rem,1.6vw,0.92rem)" }}>{step.noteJsx}</div>
              ) : (
                <p style={{ color: "var(--rust)", lineHeight: 1.78, fontSize: "clamp(0.84rem,1.6vw,0.92rem)", margin: 0, fontStyle: "italic" }}>{step.note}</p>
              )}
            </div>
          )}
        </div>
      ...
      <div style={{ backgroundColor: "var(--cream)", overflowX: "clip" }}>
        ...
        >
          <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.85, ease: [0.16, 1, 0.3, 1] }} style={{ maxWidth: "640px", textAlign: "center", margin: "0 auto" }}>
            ...
            <h1 style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "clamp(2.4rem,6vw,5rem)", lineHeight: 1.05, color: "var(--cream)", margin: "0 0 clamp(1.2rem,2.5vw,1.8rem)" }}>
              Da flor fresca<br />
              <em style={{ fontStyle: "italic", color: "var(--cream)" }}>ao quadro para sempre</em>
            </h1>
        ...
        <section aria-label="O que está incluído" style={{ padding: "clamp(52px,8vw,84px) clamp(20px,5vw,48px)", background: "linear-gradient(180deg, #F5EDE0 0%, #FAF7F0 100%)" }}>
          <div style={{ maxWidth: "900px", margin: "0 auto" }}>
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: "center", marginBottom: "clamp(28px,5vw,48px)" }}>
              ...
              <h2 style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "clamp(1.8rem,4.5vw,3rem)", color: "var(--green-d)", margin: 0, lineHeight: 1.1 }}>Tudo incluído no preço</h2>
            </motion.div>
            <div className="included-grid">
              {INCLUDED.map((item, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07, duration: 0.6 }} style={{ backgroundColor: "#fff", borderRadius: "14px", padding: "clamp(16px,2.5vw,22px)", border: "1px solid rgba(200,82,42,0.09)", boxShadow: "0 3px 14px rgba(160,60,20,0.05)", display: "flex", gap: "14px", alignItems: "flex-start" }}>
                  ...
                  <div>
                    <p style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "0.98rem", color: "var(--green-d)", margin: "0 0 4px", lineHeight: 1.2 }}>{item.label}</p>
            ...
            <div style={{ textAlign: "center", marginTop: "clamp(28px,4vw,40px)", fontSize: "0.88rem", color: "var(--mid)", lineHeight: 1.7 }}>
              Os preços começam em <strong style={{ color: "var(--green-d)" }}>300€</strong>, incluindo emolduramento e vidro museu.{" "}
          ...
          <div style={{ maxWidth: "820px", margin: "0 auto" }}>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.75 }}>
              <div style={{ textAlign: "center", marginBottom: "clamp(28px,5vw,48px)" }}>
                ...
                <h2 style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "clamp(1.7rem,4vw,2.8rem)", color: "var(--cream)", margin: "0 0 12px", lineHeight: 1.1 }}>Pagamento em três momentos</h2>
        ...
        <section aria-label="Reservar preservação" style={{ padding: "clamp(60px,10vw,100px) clamp(20px,5vw,48px)", background: "linear-gradient(140deg, #F5EDE0 0%, #FAF7F0 55%, #F0E8D8 100%)", textAlign: "center" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.75 }} style={{ maxWidth: "620px", margin: "0 auto" }}>
            ...
            <h2 style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "clamp(2rem,5.5vw,3.5rem)", color: "var(--green-d)", margin: "0 0 16px", lineHeight: 1.1 }}>Reserve a sua vaga hoje</h2>
            ...
            <div className="cta-row" style={{ marginBottom: "28px" }}>
              <a href={FORM_URL} className="btn-primary" style={{ background: "var(--rust)", borderColor: "var(--rust)", color: "var(--cream)", boxShadow: "0 4px 16px rgba(200,82,42,0.28)" }}>Preencher Formulário de Reserva</a>
