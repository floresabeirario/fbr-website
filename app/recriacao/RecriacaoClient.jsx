...
const StepCard = ({ imageSrc, number, title, desc, delay }) => (
  ...
  >
    ...
    <div style={{
      ...
      <div style={{ height: "220px", overflow: "hidden", position: "relative", backgroundColor: "#D4DECC" }}>
        <img
          src={imageSrc}
          ...
          loading="lazy"
        />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, transparent 55%, rgba(30,45,42,0.18))" }} />
      </div>
      <div style={{ padding: "24px 20px 22px", textAlign: "center" }}>
        <h3 style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "1.2rem", color: "#1E2D2A", margin: "0 0 9px", lineHeight: 1.2 }}>
          {title}
        </h3>
...
const UseCaseCard = ({ imageSrc, tag, title, desc, delay }) => (
  <motion.article
    ...
    style={{ borderRadius: "20px", overflow: "hidden", boxShadow: "0 14px 44px rgba(30,45,42,0.09)", backgroundColor: "#fff", border: "1px solid rgba(61,107,94,0.07)" }}
  >
    <div style={{ height: "250px", overflow: "hidden", position: "relative", backgroundColor: "#C8D8C0" }}>
      <img src={imageSrc} alt={title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} loading="lazy" />
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(30,45,42,0.04) 0%, rgba(30,45,42,0.6) 100%)" }} />
      <div style={{ position: "absolute", bottom: "20px", left: "20px", right: "20px" }}>
        {tag && (
          <span style={{ display: "inline-block", fontSize: "0.58rem", fontWeight: "700", letterSpacing: "2px", textTransform: "uppercase", color: "rgba(250,247,240,0.75)", border: "1px solid rgba(250,247,240,0.3)", padding: "3px 10px", borderRadius: "50px", marginBottom: "8px", fontFamily: "Roboto, sans-serif" }}>
            {tag}
          </span>
        )}
    ...
    <div style={{ padding: "20px 20px 24px" }}>
      <p style={{ color: "#5A6B60", lineHeight: 1.78, fontSize: "0.9rem", margin: 0 }}>{desc}</p>
    </div>
  </motion.article>
);
    ...
    <main style={{ backgroundColor: "#FAF7F0", overflowX: "hidden" }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        * { box-sizing: border-box; }
        :root {
          --green: #3D6B5E; --green-d: #1E2D2A; --green-l: #8BA888;
          --terra: #C4846B; --gold: #B8954A; --cream: #FAF7F0; --mid: #5A6B60;
        }
        ...
        .steps-wrapper { display: grid; grid-template-columns: 1fr; gap: 50px; }
        .step-arrow { display: none; }
        @media (min-width: 600px)  { .steps-wrapper { grid-template-columns: repeat(2,1fr); gap: 44px 22px; } }
        @media (min-width: 1024px) {
          .steps-wrapper { grid-template-columns: 1fr 40px 1fr 40px 1fr 40px 1fr; gap: 0; align-items: start; }
          .step-arrow { display: flex; align-items: flex-start; justify-content: center; padding-top: 90px; }
        }
        ...
        .btn-primary {
          ...
          text-transform: uppercase; box-shadow: 0 6px 24px rgba(61,107,94,0.28);
          transition: all 0.3s ease; text-align: center; font-family: Roboto, sans-serif;
        }
        .btn-primary:hover { background: var(--green-d); transform: translateY(-3px); box-shadow: 0 10px 32px rgba(61,107,94,0.36); }
        .btn-outline {
        ...
        .btn-whatsapp {
          ...
          background: #25D366; color: #fff; padding: 15px 34px; border-radius: 100px;
          text-decoration: none; font-weight: 600; font-size: 0.8rem; letter-spacing: 1.5px;
          text-transform: uppercase; box-shadow: 0 6px 24px rgba(37,211,102,0.3);
        ...
        .scroll-hint {
          ...
          color: rgba(250,247,240,0.5); font-size: 0.54rem; letter-spacing: 3px;
      ...
      >
        <motion.div
          aria-hidden="true"
          style={{ position: "absolute", inset: "-20%", backgroundImage: "url('/recriacao-hero.webp')", backgroundSize: "cover", backgroundPosition: "center", y: heroBgY }}
        />
        <div aria-hidden="true" style={{ position: "absolute", inset: 0, zIndex: 1, background: "linear-gradient(to bottom, rgba(15,30,26,0.35) 0%, rgba(15,30,26,0.55) 60%, rgba(15,30,26,0.75) 100%)" }} />

        <motion.div style={{ zIndex: 3, textAlign: "center", color: "#FAF7F0", padding: "0 20px", y: heroTextY, opacity: heroOpacity, maxWidth: "860px", width: "100%" }}>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.7 }}
            ...
            <a href="mailto:info@floresabeirario.pt?subject=Orçamento Recriação de Bouquet de Casamento" className="btn-primary"
              style={{ backgroundColor: "rgba(250,247,240,0.15)", border: "1px solid rgba(250,247,240,0.3)", backdropFilter: "blur(8px)" }}>
              Email
            </a>
          </motion.div>
        </motion.div>
      ...
      <section
        ...
        style={{ padding: "64px 20px 80px", background: "linear-gradient(180deg, #FAF7F0 0%, #EDF2E8 50%, #FAF7F0 100%)", position: "relative", overflow: "hidden" }}
      >
        <div aria-hidden="true" style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", fontFamily: "'TAN-MEMORIES', serif", fontSize: "clamp(70px,12vw,130px)", color: "rgba(61,107,94,0.045)", whiteSpace: "nowrap", pointerEvents: "none", lineHeight: 1, userSelect: "none" }}>
          Processo
        </div>
      ...
      <section
        ...
        style={{ padding: "56px 20px 76px", background: "linear-gradient(135deg, #EDF2E8 0%, #FAF7F0 60%, #F0EBE0 100%)", position: "relative", overflow: "hidden" }}
      >
        ...
        <div style={{ maxWidth: "840px", margin: "0 auto" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
            style={{ backgroundColor: "#fff", borderRadius: "24px", padding: "clamp(24px,5vw,48px) clamp(18px,4vw,42px)", boxShadow: "0 12px 48px rgba(30,45,42,0.07)", border: "1px solid rgba(61,107,94,0.09)", position: "relative", overflow: "hidden" }}>
            ...
            <div className="pricing-cols" style={{ marginBottom: "22px" }}>
              <div style={{ backgroundColor: "rgba(61,107,94,0.05)", borderRadius: "12px", padding: "20px 16px", border: "1px solid rgba(61,107,94,0.09)" }}>
                <h3 style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "1rem", color: "#1E2D2A", margin: "0 0 7px" }}>Flores Frescas</h3>
              ...
              <div style={{ backgroundColor: "rgba(184,149,74,0.06)", borderRadius: "12px", padding: "20px 16px", border: "1px solid rgba(184,149,74,0.11)" }}>
                <h3 style={{ fontFamily: "'TAN-MEMORIES', serif", fontSize: "1rem", color: "#1E2D2A", margin: "0 0 7px" }}>Preservação e Moldura</h3>
                <p style={{ color: "#5A6B60", fontSize: "0.85rem", lineHeight: 1.7, margin: 0 }}>
      ...
      <section
        ...
        style={{ padding: "80px 20px", background: "linear-gradient(140deg, #0F1E1A 0%, #1E3328 40%, #2D5045 100%)", position: "relative", overflow: "hidden", textAlign: "center" }}
      >
        {[
          ...
          <div aria-hidden="true" key={i} className={b.cls} style={{ position: "absolute", color: "#8BA888", width: b.w, opacity: b.op, pointerEvents: "none", animationDelay: b.d, ...(b.top ? { top: b.top } : {}), ...(b.bottom ? { bottom: b.bottom } : {}), ...(b.left ? { left: b.left } : {}), ...(b.right ? { right: b.right } : {}) }}>
            <BotanicalBloom style={{ width: "100%" }} />
          </div>
        ))}
